...
console.log("\n1. Address resolution from MAIL_DOMAIN alone");
{
  ...
  ok(
    `MAIL_FROM defaults to no-reply@${DOMAIN}`,
    m.MAIL_FROM === `Magma Legal Practitioners <no-reply@${DOMAIN}>`,
    m.MAIL_FROM,
  );
  ok(
  ...
  ok(
    "MAIL_NOTIFY_TO falls back to MAIL_REPLY_TO",
    m.MAIL_NOTIFY_TO === m.MAIL_REPLY_TO,
    m.MAIL_NOTIFY_TO,
  );
  console.log(
    `        -> mail would be sent from "${m.MAIL_FROM}", replies to ${m.MAIL_REPLY_TO}, notifications to ${m.MAIL_NOTIFY_TO}`,
  );
}
...
}
{
  ...
  ok(
    "STAFF_EMAIL is accepted as an alias for MAIL_NOTIFY_TO",
    m.MAIL_NOTIFY_TO === "staff@gmail.com",
    m.MAIL_NOTIFY_TO,
  );
}
...
console.log("\n4. sendEmail with a key: the request Resend actually receives");
{
  ...
  ok(
    ...
    captured?.auth === "Bearer re_test_key_123",
    captured?.auth,
  );
  ...
  ok(
    "reply_to uses Resend's snake_case key",
    captured?.body.reply_to === "visitor@example.com",
...
console.log("\n6. A rejected send surfaces Resend's own message");
{
  ...
  let msg = "";
  try {
    await m.sendEmail({ to: "a@b.com", subject: "x", html: "<p>x</p>" });
  } catch (e: any) {
...
console.log("\n7. Inbound webhook signature (Svix HMAC-SHA256)");
{
  ...
  const good = new Headers({
    "svix-id": id,
    "svix-timestamp": ts,
    "svix-signature": sign(id, ts, body, keyBytes),
  });
  ok(
    "a correctly signed delivery is accepted",
    m.verifyResendWebhook(body, good).ok === true,
    JSON.stringify(m.verifyResendWebhook(body, good)),
  );

  const alias = new Headers({
    "webhook-id": id,
    "webhook-timestamp": ts,
    "webhook-signature": sign(id, ts, body, keyBytes),
  });
  ...
  ok(
    "a second signature version in the header still matches",
    m.verifyResendWebhook(body, multi).ok === true,
  );
  ...
  ok(
    ...
    m.verifyResendWebhook(
      body,
      new Headers({ "svix-id": id, "svix-timestamp": ts, "svix-signature": wrongKey }),
    ).ok === false,
  );

  const oldTs = (Math.floor(Date.now() / 1000) - 3600).toString();
  const replay = m.verifyResendWebhook(
    body,
    new Headers({
      "svix-id": id,
      "svix-timestamp": oldTs,
      "svix-signature": sign(id, oldTs, body, keyBytes),
    }),
  );
  ...
  ok(
    "missing headers are named, not guessed",
    missing.ok === false && /svix-timestamp/.test((missing as any).reason),
...
console.log("\n8. What /api/health says about the webhook secret");
{
  const m = await load({ MAIL_DOMAIN: DOMAIN });
  const good =
    "whsec_" + Buffer.from("32-bytes-of-signing-key-material", "utf8").toString("base64");
  ...
  ok(
    "the real secret is reported as fine",
    m.describeWebhookSecret(good).startsWith("looks right"),
  );
  ok(
    "an API key in this slot is caught",
    /Resend API key/.test(m.describeWebhookSecret("re_ABC123def456GHI789jkl")),
  );
  ok(
    "a Supabase key in this slot is caught",
    /Supabase/.test(m.describeWebhookSecret("sb_publishable_AbC123")),
  );
  ...
  ok(
    "a truncated secret is caught",
    /SUSPECT/.test(
      m.describeWebhookSecret("whsec_" + Buffer.from("short", "utf8").toString("base64")),
    ),
  );
  ok(
    "unset is not reported as an error",
    /only needed to receive/.test(m.describeWebhookSecret(undefined)),
  );
}
{
  const m = await load({ MAIL_DOMAIN: DOMAIN });
  ...
  ok(
    "an unset secret says which variable to set",
    r.ok === false && r.reason.includes("RESEND_WEBHOOK_SECRET"),
    JSON.stringify(r),
  );
}
