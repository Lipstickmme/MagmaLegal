...
// Auth failures.
{
  const d = describeError(
    new AuthError("Anonymous sign-ins are disabled", 422, "anonymous_provider_disabled"),
  );
  ok(
    "anonymous sign-ins disabled is classified so the chat can name the fix",
    d.kind === "anonymous-disabled",
    d.kind,
  );
}
{
  const got = readableError({
    error: "invalid_grant",
...
for (const [name, value] of [
  ...
  ok(
    `${name} still yields a sentence`,
    typeof got === "string" && got.length > 0 && got !== "[object Object]",
    got,
  );
}
...
for (const message of [
  ...
  ok(
    `"${message}" is classified as a network failure`,
    describeError({ message }).kind === "network",
  );
}
